...
2. Load the final structure from assets/report-template.md and use the Task 5 requirements in this SKILL.md
3. Execute report assembly using only document and spreadsheet capabilities actually listed by the current runtime:
   - Use an available document-generation tool or library to create and manipulate the Word document
   - Use an available spreadsheet-reading tool or library to read Excel data from Task 2/3
   ...
   - Insert Task 4 .png chart files throughout using the selected document capability
   - Create text-dense report with charts interspersed every 200-300 words
4. Save and deliver final DOCX report

**Key Principles**:
- Do not assume a named DOCX or XLSX tool exists; select only capabilities exposed by the current runtime and stop Task 5 if none can produce a verifiable DOCX
...
Assistant: "Starting Task 5: Report Assembly for Tesla
...
- **assets/report-template.md** - Final report structure and formatting checklist
