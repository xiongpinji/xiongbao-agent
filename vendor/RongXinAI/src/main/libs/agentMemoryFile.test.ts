...
test('resolveMemoryFilePath rejects an empty workspace', () => {
  expect(() => resolveMemoryFilePath('')).toThrow('Agent workspace is required');
});
...
test('resolveMemoryFilePath: rejects an undefined workspace', () => {
  expect(() => resolveMemoryFilePath(undefined)).toThrow('Agent workspace is required');
});
