...
def _ensure_agent_acl_schema(db: DatabasePool) -> None:
    """Idempotent helper for migration 015.

    Folded into the Python repair path so partially-upgraded databases
    (skipped migration files, identity rebuilds) still receive the
    ``agent_acl`` table. Mirrors the CREATE TABLE in 015_agent_acl.sql.
    """
    if not _table_exists(db, "agents"):
        return
    if not _table_exists(db, "agent_acl"):
        with db.connect() as conn:
            if db.dialect == "postgresql":
                conn.execute(
                    """
            ...
            # Backfill: pre-existing ``is_shared = 1`` rows become the
            # public-readable sentinel so the new model is a strict
            ...
            conn.execute(
                ...
                "FROM agents WHERE is_shared = 1"
            )
            conn.execute(
                "UPDATE _schema_version SET version = ? WHERE version < ?",
                (15, 15),
            )
...
def _ensure_credits_schema(db: DatabasePool) -> None:
    """Idempotent helper for migration 016.

    Creates ``credit_account`` (one row per user) and ``credit_ledger``
    (append-only audit log). Mirrors the CREATE TABLE statements in
    016_credits.sql so partial upgrades / rebuilds still receive the new
    tables.
    """
    if not _table_exists(db, "users"):
        return
    pg = db.dialect == "postgresql"
    int_type = "BIGINT" if pg else "INTEGER"
    if not _table_exists(db, "credit_account"):
        with db.connect() as conn:
            conn.execute(
                f"""
                CREATE TABLE credit_account (
                  user_id                    {int_type} PRIMARY KEY REFERENCES users(id) ON DELETE CASCADE,
                  balance                    {int_type} NOT NULL DEFAULT 0,
                  total_earned               {int_type} NOT NULL DEFAULT 0,
                  total_spent                {int_type} NOT NULL DEFAULT 0,
                  tier                       TEXT    NOT NULL DEFAULT 'free'
                                             CHECK (tier IN ('free', 'basic', 'pro', 'enterprise')),
                  tier_name                  TEXT    NOT NULL DEFAULT 'Free',
                  monthly_allowance          {int_type} NOT NULL DEFAULT 0,
                  monthly_used               {int_type} NOT NULL DEFAULT 0,
                  monthly_window_start       {int_type},
                  next_reset_at              {int_type},
                  updated_at                 {int_type} NOT NULL
                )
                """
            )
    if not _table_exists(db, "credit_ledger"):
        with db.connect() as conn:
            if pg:
                id_column = f"{int_type} GENERATED BY DEFAULT AS IDENTITY PRIMARY KEY"
            else:
                # SQLite: only a plain ``INTEGER PRIMARY KEY`` may carry
                # ``AUTOINCREMENT``; declaring the column as ``INTEGER`` is
                # already a rowid alias, so we don't need ``BIGINT`` here.
...
def _apply_sqlite_migration(db: DatabasePool, version: int, path: Path) -> None:
    ...
    if version == 14:
        ...
        return
    if version == 15:
        _ensure_agent_acl_schema(db)
        with db.connect() as conn:
            conn.execute("UPDATE _schema_version SET version = ?", (version,))
        return
    if version == 16:
        _ensure_credits_schema(db)
        with db.connect() as conn:
            conn.execute("UPDATE _schema_version SET version = ?", (version,))
        return
    sql = path.read_text(encoding="utf-8")
