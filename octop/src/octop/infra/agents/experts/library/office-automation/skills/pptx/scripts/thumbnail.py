...
def convert_to_images(pptx_path: Path, temp_dir: Path) -> list[Path]:
    ...
    # Try pdftoppm first, fall back to pdf2image (cross-platform)
    try:
        ...
        # pdftoppm not available (e.g. Windows without Poppler); use pdf2image
        try:
            from pdf2image import convert_from_path as _pdf_convert

            images = _pdf_convert(str(pdf_path), dpi=CONVERSION_DPI)
            for idx, img in enumerate(images, start=1):
                out = temp_dir / f"slide-{idx:02d}.jpg"
                img.save(str(out), "JPEG")
        except ImportError:
            raise RuntimeError(
                "Image conversion failed: pdftoppm not found and "
                "pdf2image package is not installed. "
                "Install poppler or run: pip install pdf2image"
            ) from None
    else:
        if result.returncode != 0:
            stderr = (result.stderr or "").strip() or "unknown pdftoppm error"
